...
- Output: `32/32 build · 10/10 pipeline · tự kiểm ĐẠT` (sau phase sub-cells).
...
## ✅ Sub-cells Phase 1 — 1D split bên trong open-back / open-nobk (2026-05-20)

Founder yêu cầu mở rộng: 1 ô lớn có thể chia thành nhiều "vách con" (sub-cells) trong
cùng 1 hàng hoặc 1 cột. Phase 1 áp dụng cho cha **open-back** và **open-nobk**.

**Quyết định cốt lõi:**
- Depth 1 cấp (KHÔNG sub-sub).
- Shape 1D — H (1×N chia ngang) hoặc V (N×1 chia dọc); user chọn ngang/dọc khi bấm "Chia ô".
- Sub-grid chia ĐỀU (manual size per slot là Phase 2).
- **open-back cha** → 1 tấm hậu CHUNG (cw × ch). Vách sub ngắn hơn `D − T_BACK`
  để chừa hậu. Sub được dùng đủ 4 loại.
- **open-nobk cha** → KHÔNG hậu chung. Sub-cell drawer **fallback sang door**
  (drawer cần khoảng hậu để bắt ray, open-nobk không cung cấp).
- Cell cha thành CONTAINER (value `'split'`); loại gốc lưu trong `SubEntry.parentKind`
  để merge restore đúng.

**Engine mở rộng (additive — sản phẩm cũ không cần thay đổi):**
1. **`src/configurator/subgrid.ts`** (file mới):
   - `interface SubEntry { dir: 'H'|'V'; parentKind; sizes[]; cells[] }`
   - `parseSubCells(raw): Map<key, SubEntry>` · `encodeSubCells(map): string`
   - `reconcileSubCells(raw, parentCells, colWidths, rowHeights, parentTypeBefore,
     fallbackForType, cellMin, T): string` — drop entry vi phạm size, fallback loại
     slot theo `entry.parentKind`.
   - Format: `<r>_<c>=<dir><N>/<parentKind>:<sizes>:<cells>` (entries phân tách `|`).
   - File hằng số `subgrid-constants.ts` (CELL_MIN_DEFAULT, T_DEFAULT).
2. **`src/configurator/types.ts`** (+3 field optional + 1 variant):
   - `Parameter.subGridAllowed?: boolean`
   - `Parameter.subContainerValue?: string` (mặc định `'split'`)
   - `Parameter.subGridSourceId?: string` (id param lưu subCells)
   - `cellVariant: 'type' | 'color' | 'subgrid'` (subgrid = carrier, no direct UI)
3. **`src/configurator/Configurator.tsx`**:
   - `CellGridControl` thêm prop `allValues` + `onChangeMulti` để lookup subCells &
     batch update.
   - Ô có value 'split' → render mini sub-grid INLINE bằng CSS grid lồng (component
     `SubGridRender`).
   - CellMenu thêm nút "Chia ngang"/"Chia dọc" cho open-back/open-nobk; "+1 ô"/"−1 ô"/
     "Gộp ô lại" cho 'split' (component `buildSubActions`).
   - intentValues/resolvedValues: SKIP reconcile cho `cellVariant='subgrid'` (passthrough).
   - Setter mới `setParamBatch(updates: Record<string, ...>)` — atomic multi-param
     update. **Cần thiết** vì splitCell phải set `cells` + `subCells` cùng 1 setValues
     (race condition nếu set riêng → normalize chạy 2 lần với prev không nhất quán).
4. **`products/tu-ke/dna.ts`**:
   - Extract helper `buildOneCell(ctx, parts, counters, frameMaterial, slideSize)` —
     dùng cho cell thường + sub-cell, đầy đủ 4 nhánh open-back/nobk/door/drawer.
   - Vòng lặp build chính: phân nhánh `if (type === SPLIT)` → vẽ vách sub + tấm hậu
     chung (nếu parentKind=open-back) + gọi `buildOneCell` cho mỗi sub-cell với scope
     đã shift toạ độ; sub-cell có `hasSharedBack=true` khi cha có hậu chung → không
     vẽ thêm tấm lưng riêng.
   - 2 param ẩn `subCells`/`subCellColors` (cellVariant='subgrid') + flag
     `subGridAllowed: true` cho cells.
   - `normalizeValues` gọi `reconcileSubCells` (drop entry vi phạm + cells defensive
     reset về DEFAULT_CELL nếu cha 'split' nhưng subCells mất entry).
5. **`scripts/validate-dna.ts`**:
   - Pipeline runner SKIP reconcile cho cellVariant='subgrid' (mirror UI).
   - 4 case mới: H 1×3 cánh đơn (open-back, hậu chung); V 3×1 (drawer + open-back,
     hậu chung); H 1×2 open-nobk cha (drawer→door, không hậu chung); rollback khi
     cha quá nhỏ.

**Đã verify trên live preview:**
- Default 18.646.803₫ giữ nguyên.
- Bấm ô open-nobk (Cột 2, tầng 3) → menu xuất hiện "Chia ngang"/"Chia dọc".
- Click "Chia ngang" → 2 sub-cell render inline (DOM `Sub-cell 1`, `Sub-cell 2`),
  giá cập nhật 18.866.412₫.
- Click sub-cell 1, chọn "Cánh" → giá update 19.005.520₫.
- tsc pass · validator `32/32 build + 10/10 pipeline`.

**Lưu ý kiến trúc:**
- Pattern `intentValues` vs `resolvedValues` mở rộng tốt cho subgrid carrier — chỉ
  cần skip reconcile, pipeline còn lại bất biến.
- `setParamBatch` là engine util mở rộng cho transactional UI; future plugins cần
  multi-param changes (vd undo/redo) sẽ dùng được.
- `parentKind` trong SubEntry encoding cho phép cell cha biến mất (value='split') mà
  vẫn restore được khi merge — đây là state encapsulation đúng.

**Phase 2 (chưa làm):** manual size per slot; sub-cells trong door cha (kệ trong tủ
có cánh); sub-sub-cells (depth 2) nếu nghiệp vụ cần.
